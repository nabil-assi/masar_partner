...
const outcomes = [
  {
    ...
    description: "نحوّل الخطوات المتكررة إلى مسارات واضحة تقلل الأخطاء وتمنح الفريق وقتًا أكبر للتركيز على القيمة.",
    metric: "أتمتة",
  ...
  },
  {
    ...
    description: "نوفر لوحات وتقارير تساعد الإدارة على متابعة الأداء والقرارات بدقة أكبر عبر بيانات موثوقة.",
    metric: "قياس",
  ...
  },
  {
    ...
    description: "نصمم واجهات منظمة وسلسة تجعل التفاعل اليومي أسهل، سواء للعميل أو لفريق العمل الداخلي.",
    metric: "UX",
...
const engagementModels = [
  {
    title: "بناء حل جديد",
    description: "نحوّل الفكرة أو الحاجة التشغيلية إلى منصة أو تطبيق جاهز للإطلاق مع خارطة مراحل واضحة.",
    points: ["تحليل نطاق العمل", "تصميم تجربة المستخدم", "تطوير واختبار وإطلاق"],
  },
  {
    title: "تطوير نظام قائم",
    description: "نراجع النظام الحالي ونحسن الأداء والتجربة والتكاملات بدون تعطيل سير العمل.",
    points: ["تقييم تقني وتجربة", "تحسين الواجهات والأداء", "ربط الأنظمة والبيانات"],
  },
  {
    title: "شراكة تشغيل ودعم",
    description: "نرافق الفريق بعد الإطلاق بالقياس، التحسين، الدعم الفني، وتطوير الإصدارات القادمة.",
        ...
        <div className="website-container grid items-center gap-12 lg:grid-cols-[1fr_1.05fr] lg:gap-16">
          <div className="order-1 text-right lg:col-start-1 lg:row-start-1">
            ...
            <p className="mt-6 max-w-xl text-base leading-8 text-slate-600 sm:text-lg sm:leading-9">
              نصمم ونطور المنصات والتطبيقات وأنظمة الأتمتة التي تربط العمليات وتحسن تجربة العملاء، من الفكرة إلى الإطلاق والتطوير المستمر.
            </p>
        ...
        <div className="website-container">
          ...
          <div className="mt-14 grid gap-5 lg:grid-cols-3">
            {outcomes.map((outcome) => {
              ...
              return (
                <article key={outcome.title} className="group relative overflow-hidden rounded-[1.75rem] border border-slate-200/80 bg-gradient-to-br from-white via-slate-50 to-blue-50/70 p-7 text-right shadow-[0_14px_40px_rgba(18,69,132,0.05)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_65px_rgba(18,69,132,0.10)]">
                  ...
                  <p className="mt-6 text-xs font-bold tracking-[0.26em] text-slate-400">{outcome.metricLabel}</p>
                  <h3 className="mt-3 text-xl font-extrabold text-[#071b4e]">{outcome.title}</h3>
                  <p className="mt-4 text-sm leading-7 text-slate-600">{outcome.description}</p>
        ...
        <div className="website-container">
          <div className="mx-auto w-full max-w-6xl rounded-[1.75rem] border border-slate-200/70 bg-white p-6 sm:p-8 lg:p-10">
            <div className="text-center">
              <p className="text-[11px] font-extrabold uppercase tracking-[0.28em] text-[#075dc7]">أرقام تعكس أثر الحلول</p>
              <h3 className="mt-2 text-2xl font-extrabold text-[#071b4e] sm:text-[1.8rem]">نتائج واضحة، وسمعة قوية</h3>
              <p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-slate-600">نحوّل الأعمال إلى حلول رقمية مدروسة، تقدم نتائج ملموسة مع كل مرحلة.</p>
            </div>

            <div className="mt-8 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
              {trustStats.map((stat) => (
                <div key={stat.label} className="rounded-[1.25rem] bg-slate-50/80 px-4 py-5 text-center">
                  <div className="text-3xl font-extrabold tracking-[-0.03em] text-[#075dc7] sm:text-4xl">{stat.value}</div>
        ...
        <div className="website-container">
          ...
          <div className="mt-14 grid gap-5 lg:grid-cols-3">
            {caseStudies.map((item) => (
              <Link key={item.title} href="/portfolio" className="group overflow-hidden rounded-[1.75rem] border border-slate-200/80 bg-white shadow-[0_18px_50px_rgba(18,69,132,0.07)] transition hover:-translate-y-1 hover:shadow-[0_28px_75px_rgba(18,69,132,0.12)]">
                ...
                <div className="p-6 text-right">
                  <span className="text-xs font-extrabold text-[#075dc7]">{item.category}</span>
                  <h3 className="mt-3 text-xl font-extrabold text-[#071b4e]">{item.title}</h3>
                  <div className="mt-5 space-y-4 text-sm leading-7 text-slate-600">
                    <p><strong className="text-slate-800">التحدي: </strong>{item.challenge}</p>
                    <p><strong className="text-slate-800">النتيجة: </strong>{item.result}</p>
                  </div>
                  <span className="mt-6 inline-flex items-center gap-2 text-sm font-extrabold text-[#075dc7]">
                    مشاهدة الأعمال <ArrowLeft className="h-4 w-4 transition-transform group-hover:-translate-x-1" />
                  </span>
                </div>
              </Link>
            ))}
          ...
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
            <div className="text-right">
              ...
              <p className="mt-5 text-base leading-8 text-slate-600">
                سواء كنت تبدأ من فكرة، تملك نظاماً يحتاج تطويراً، أو تبحث عن شريك تقني طويل الأمد، نبني طريقة تعاون واضحة تناسب المرحلة والميزانية.
              </p>
              <Link href="/contact" className="mt-8 inline-flex items-center gap-2 text-sm font-extrabold text-[#075dc7]">
            ...
            <div className="grid gap-4">
              {engagementModels.map((model, index) => (
                <article key={model.title} className="rounded-3xl border border-slate-200/80 bg-white p-6 text-right shadow-[0_14px_40px_rgba(18,69,132,0.05)]">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <span className="text-sm font-black text-blue-300">{String(index + 1).padStart(2, "0")}</span>
                      <h3 className="mt-2 text-xl font-extrabold text-[#071b4e]">{model.title}</h3>
                    </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {model.points.map((point) => (
                      <span key={point} className="rounded-full bg-slate-50 px-3 py-1.5 text-xs font-bold text-slate-600">
                        {point}
                      </span>
                    ))}
                  </div>
